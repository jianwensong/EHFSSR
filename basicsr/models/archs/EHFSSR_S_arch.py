...
class EHFSSR_S(nn.Module):
    def __init__(self, up_scale=4, dim=48, groups=5,num=6):
        super(EHFSSR_S, self).__init__()
...
class WSAttention(nn.Module):
    def __init__(self,channels,window_sizes = [8,8],down_rates = [1,2],shift = False):
        ...
        self.wsize = window_sizes[0]
        self.heads = len(window_sizes)
        self.depthwise  = nn.Conv2d(channels,channels,3,1,1,groups=channels)
        self.local = nn.Conv2d(channels, channels//2, kernel_size=1)
        self.CA = effCA(channels,3)
        self.to_qk = nn.Conv2d(channels, channels, kernel_size=1)
        self.to_v = nn.Conv2d(channels, channels//2, kernel_size=1)
        self.scale = (channels//2) ** -0.5
        self.sa = nn.Sequential(nn.Conv2d(channels//2,1,1),nn.Sigmoid())
    ...
    def forward(self, x0):
        ...
        x = self.Layernorm(x0)
        x1 = self.depthwise(x)
        v = self.to_v(self.CA(x1) * x)
        x1 = self.local(x1)
        qk = self.to_qk(x)
        if self.shift:
            qk = torch.roll(qk,shifts=(-self.wsize//2, -self.wsize//2), dims=(2,3))
...
if __name__ == "__main__":
    net = EHFSSR_S(up_scale=4,  dim=48, groups=5,num=6)
