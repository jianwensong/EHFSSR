...
from basicsr.utils import FileClient, imfrombytes, img2tensor, padding,h5np
import os
import numpy as np
import h5py
import random
...
class PairedImageSRLRDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            import os
            self.gtlist = os.listdir(self.gt_folder)
            self.lqlist = os.listdir(self.lq_folder)
            nums_lq = len(self.lqlist)
            nums_gt = len(self.gtlist)
    ...
    def __getitem__(self, index):
        ...
        gt_path_L = os.path.join(self.gt_folder,self.gtlist[index],'hr0.png')
        gt_path_R = os.path.join(self.gt_folder, self.gtlist[index],'hr1.png')
        ...
        lq_path_L = os.path.join(self.lq_folder,self.gtlist[index],'lr0.png')
        lq_path_R = os.path.join(self.lq_folder, self.gtlist[index],'lr1.png')
...
class PairedStereoImageDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        with h5py.File(self.gt_folder, 'r') as f:
          self.nums = len(f['hr_L'])  
    def open_hdf5(self):
        self.h5_file = h5py.File(self.gt_folder, 'r')
    def __getitem__(self, index):
        ...
        gt_path_L = self.h5_file['hr_L'][str("%06d" % index)]
        gt_path_R = self.h5_file['hr_R'][str("%06d" % index)]
        
        img_gt_L = h5np(gt_path_L, float32=True)
        img_gt_R = h5np(gt_path_R, float32=True)


        lq_path_L = self.h5_file['lr_L'][str("%06d" % index)]
        lq_path_R = self.h5_file['lr_R'][str("%06d" % index)]

        img_lq_L = h5np(lq_path_L, float32=True)
        img_lq_R = h5np(lq_path_R, float32=True)
        ...
        return {
            'lq': img_lq,
            'gt': img_gt,
            'lq_path': str("%06d" % index),
            'gt_path': str("%06d" % index),
        }
...
class PairedImageStereoDatasetSingle(data.Dataset):
    """Paired image dataset for image restoration.
    ...
    def __init__(self, opt):
        super(PairedImageStereoDatasetSingle, self).__init__()
        self.opt = opt
        # file client (io backend)
        self.file_client = None
        self.io_backend_opt = opt['io_backend']
